...
export function loadGamesList(): Promise<Game[]> {
  return axios
    .get(API_GAMES_PATH, { ...authService.withAuth() }) //.then(({data}) => data);
    .then(() => [
      {
        id: "ssf23f2",
        owner: { id: "1", login: "Galya" },
        slotsCount: 4,
        playersCount: 2,
      },
      {
        id: "u3i4tg",
        owner: { id: "1", login: "Misha" },
        slotsCount: 2,
        playersCount: 0,
      },
      {
        id: "kf349w2",
        owner: { id: "1", login: "Asylbek" },
        slotsCount: 4,
        playersCount: 1,
      },
      {
        id: "k95jthire",
        owner: { id: "1", login: "Ebtygydyk" },
        slotsCount: 3,
        playersCount: 1,
      },
    ]);
...
export function createGame(game: GameBlank): Promise<Game> {
  return axios
    .post(API_GAME_PATH, game, { ...authService.withAuth() })
    .then(({ data }) => data);
}
